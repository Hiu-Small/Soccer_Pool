...
bool Field::isInGoal1(sf::Vector2f pos) const {
    // ---> FIX: Nhân hệ số 0.8f để đồng bộ với hitbox vật lý. 
    // Bóng chỉ cần chui phần lõi qua vạch là tính bàn ngay, cảm giác sẽ rất nhạy!
    if (pos.x + (BALL_RADIUS * 0.8f) >= FIELD_MARGIN_X) return false;
...
bool Field::isInGoal2(sf::Vector2f pos) const {
    // ---> FIX: Tương tự cho gôn 2
    if (pos.x - (BALL_RADIUS * 0.8f) <= FIELD_WIDTH - FIELD_MARGIN_X) return false;
    float gy = GOAL_Y_OFFSET;
